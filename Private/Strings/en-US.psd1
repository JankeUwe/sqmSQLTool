@{
    ...
    # What the wait type actually means (HTML report tooltip) - independent of the
    # threshold-based Recommendation above, which only judges whether THIS value is a problem.
    'WaitDef_PAGEIOLATCH_SH'       = 'A task is waiting for a shared latch on a data-file page that is currently being read from disk into the buffer pool - i.e. a physical read is in progress. Normal in small amounts; a high average points to slow storage.'
    'WaitDef_PAGEIOLATCH_EX'       = 'Like PAGEIOLATCH_SH but for a page held under an exclusive latch, typically during a physical write or the first read into an empty buffer.'
    'WaitDef_PAGEIOLATCH_UP'       = 'Wait for an update latch on a data-file page during physical I/O, often seen in tempdb PFS/GAM/SGAM contention.'
    'WaitDef_WRITELOG'             = 'Wait for a transaction log record to be flushed (hardened) to disk by the log writer - a commit is waiting on log I/O.'
    'WaitDef_IO_COMPLETION'        = 'Generic wait for a non-buffer-pool disk I/O to finish, e.g. reading a backup file or a large sequential scan read-ahead.'
    'WaitDef_ASYNC_IO_COMPLETION'  = 'Wait for a single asynchronous I/O to complete, most commonly during backup, restore, or DBCC CHECKDB.'
    'WaitDef_LCK_M_X'              = 'Wait to acquire an exclusive (X) lock - blocked because another session already holds a conflicting lock on the same resource.'
    'WaitDef_LCK_M_S'              = 'Wait to acquire a shared (S) lock - typically a reader blocked behind a writer holding an exclusive lock.'
    'WaitDef_LCK_M_U'              = 'Wait to acquire an update (U) lock, taken before SQL Server upgrades to an exclusive lock (e.g. during UPDATE statements).'
    'WaitDef_LCK_M_IX'             = 'Wait to acquire an intent-exclusive (IX) lock, needed before taking a more granular X lock further down the hierarchy (e.g. a row lock under a table).'
    'WaitDef_LCK_M_IS'             = 'Wait to acquire an intent-shared (IS) lock, needed before taking a more granular S lock further down the hierarchy.'
    'WaitDef_CXPACKET'             = 'Wait caused by parallel query execution - one thread of a parallel plan waiting for other threads to catch up or hand off rows via an exchange operator.'
    'WaitDef_CXCONSUMER'           = 'The benign counterpart of CXPACKET (SQL 2016+): a parallel consumer thread waiting for work. Large values are normal for parallel plans and only matter together with high CXPACKET.'
    'WaitDef_CXSYNC_PORT'          = 'Wait while parallel threads synchronize at an exchange operator port (opening/closing a parallel region) - distinct from the row-transfer wait of CXPACKET.'
    'WaitDef_RESOURCE_SEMAPHORE'   = 'Wait for a query memory grant - the requested workspace memory (sorts/hashes) is not yet available because other queries are using the memory grant pool.'
    'WaitDef_RES_SEM_COMPILE'      = 'Wait for one of the limited concurrent-compile memory slots - too many query compilations/recompiles competing for compile memory at once.'
    'WaitDef_CMEMTHREAD'           = 'Wait for a protected/synchronized memory object (e.g. plan cache, memory broker) - often points to NUMA imbalance or heavy concurrent memory-cache churn.'
    'WaitDef_SOS_SCHEDULER_YIELD'  = 'A task voluntarily yielded the CPU (SQL Server''s cooperative scheduler) and is waiting for its turn again - a large signal-wait share here indicates CPU pressure.'
    'WaitDef_THREADPOOL'           = 'Wait for a free worker thread - all configured worker threads are busy. Unlike most waits this is never harmless and can make the instance unresponsive.'
    'WaitDef_PAGELATCH_EX'         = 'Wait for an exclusive latch on a buffer-pool page already in memory (no physical I/O) - common in tempdb allocation-page contention or "last page insert" hotspots.'
    'WaitDef_PAGELATCH_SH'         = 'Wait for a shared latch on an in-memory buffer-pool page - frequently tempdb PFS/SGAM/GAM contention under high object creation/deletion rates.'
    'WaitDef_PAGELATCH_UP'         = 'Wait for an update latch on an in-memory page, typically from concurrent inserts into the same last page of an ascending index ("hot" last page).'
    'WaitDef_LATCH_EX'             = 'Wait for an exclusive latch on a non-page internal structure - the subtype in wait_resource identifies what is contended.'
    'WaitDef_LATCH_SH'             = 'Wait for a shared latch on a non-page internal structure, e.g. metadata or cache structures.'
    'WaitDef_DBMIRROR_EVENTS_QUEUE'= 'A Database Mirroring worker waiting for the next event in its internal queue - grows with mirroring/network latency between partners.'
    'WaitDef_DBMIRRORING_CMD'      = 'Wait while a mirroring command is being processed/acknowledged by the partner - reflects mirroring send/ack latency.'
    'WaitDef_ASYNC_NETWORK_IO'     = 'Wait for the client application to consume (ACK) results SQL Server has already sent - despite the name this is almost always a slow/chatty client, not the network itself.'
    'WaitDef_OLEDB'                = 'Wait during a call into the OLE DB provider layer - covers linked-server queries and some backup/restore or full-text operations. Usually reflects whatever remote/external component is being called.'
    'WaitDef_BACKUPBUFFER'         = 'A backup/restore worker waiting for a free in-memory buffer to fill with data - a normal, expected wait while a backup or restore is running.'
    'WaitDef_BACKUPIO'             = 'A backup/restore worker waiting on I/O to the backup device (disk or network share) - a high average points to slow backup target storage or network.'
    'WaitDef_BACKUPTHREAD'         = 'Wait for a backup/restore worker thread to become available - only relevant while backups are actively running.'
    'WaitDef_MSQL_XP'              = 'Wait while an extended stored procedure (xp_*) executes - the duration reflects whatever the extended proc itself is doing.'
    'WaitDef_BUFFERPOOL_SCAN'      = 'Wait during backup/restore operations while the relevant buffer pool pages are flushed to disk to keep an online backup consistent.'
    'WaitDef_WAIT_ON_SYNC_STATISTICS_REFRESH' = 'A query waiting for a synchronous auto-update of statistics to finish before it can run - consider AUTO_UPDATE_STATISTICS_ASYNC if this happens often.'
    'WaitDef_Generic'              = 'No built-in explanation for this wait type yet - see Microsoft''s sys.dm_os_wait_stats documentation or the SQLskills wait type library for details.'
